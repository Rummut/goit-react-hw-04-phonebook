...
import { ContactForm } from './contact-form/ContactForm.hook';
...
export const App = () => {
  const [contacts, setContacts] = useState(() => {
    return (
      JSON.parse(localStorage.getItem('array-contacts')) ?? [
  ...
  const handleContactsAdd = newContact => {
    ...
    setContacts(prevContact => [...contacts, newContact]);
  };

  const handleDelete = listDeleteId => {
    setContacts(contacts.filter(contact => contact.id !== listDeleteId));
  };
